...
import { commands, window, ExtensionContext, Uri, workspace, TreeItem, TreeItemCollapsibleState, Disposable, TreeDataProvider, ThemeIcon, ProgressLocation, FileDecoration, EventEmitter, Event, Command } from 'vscode';
...
export function activate({ subscriptions }: ExtensionContext) {
	...
	const previewWelcome = commands.registerCommand('dure.welcome', () => {
		...
		window.showQuickPick(pathKeys.map(key => ({
			label: key,
	...
	class SyncFile extends TreeItem {
		// iconPath = {
	...
	let generatorRandomTestData = (index: number, icon: string = "circle-large-outline") => new SyncFile(
		`测试${Math.ceil(Math.random() * 1000)}`,
		bool() ? 1 : 0,
		new ThemeIcon(icon),
		{ title: "asd", command: "dure.addSyncFile", arguments: [index] },
		`${bool() ? 1 : 0}`
	);

	let _fileData = (): SyncFile[] => Array.from({ length: 0 }, (el, i): SyncFile => generatorRandomTestData(i));

	let generateData: SyncFile[] = _fileData();
	...
	class FileTreeData {
		...
		getChildren(el: SyncFile): SyncFile[] {
			console.log(el);
	...
	treeData.onDidChangeTreeData((data: any) => {
		console.dir('09090909090');
		console.dir(data);
	});

	window.registerTreeDataProvider("sync-files", treeData);

	commands.registerCommand('dure.addSyncFile', (index) => {
		console.dir('--------------');
		// console.dir(el);
		// console.dir(el2);
	...
	const test = commands.registerCommand('dure.test', () => {
		...
		let treeData: TreeDataProvider<any> = {
			...
			getTreeItem(el: Record<string, any>) {
				// console.dir(el);
				console.dir('99999999999999');
				let treeItem = new TreeItem({ label: `测试${Math.ceil(Math.random() * 1000)}`, highlights: [[0, el.name.length]] }, el.index)
				treeItem.iconPath = new ThemeIcon("circle-large-outline");
				// treeItem.iconPath = Uri.joinPath(Uri.parse(".."), 'resources/sync.svg');
				// treeItem.contextValue = "asdasdas";
				treeItem.tooltip = "asfasfasfa";
				treeItem.command = {
		...
		let tree = window.createTreeView("sync-files", {
